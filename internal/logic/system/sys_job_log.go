...
func (s *sSysJobLog) GetJobLog(ctx context.Context, id int) (out *model.SysJobLogOut, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		err = dao.SysJobLog.Ctx(ctx).Where("del_flag", 0).Where(dao.SysJobLog.Columns().Id, id).Scan(out)
		if err != nil {
...
func (s *sSysJobLog) JobLogList(ctx context.Context, input *model.GetJobLogListInput) (total int, out []*model.SysJobLogOut, err error) {
	err = g.Try(ctx, func(ctx context.Context) {
		...
		m = m.Where("del_flag", 0) // 只查询未删除的记录
		if input != nil {
			if input.Status != "" {
		...
		err = m.Page(input.PageNum, input.PageSize).Scan(&out)
		if err != nil {
...
func (s *sSysJobLog) DelJobLogByIds(ctx context.Context, ids []int) (err error) {
	...
	_, err = dao.SysJobLog.Ctx(ctx).Data("del_flag=1").Data(g.Map{
		"delete_by": userId,
		"delete_at": gtime.Now(),
